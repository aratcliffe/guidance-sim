...
function simulate(map, config, version) {
  ...
  map.flyTo({ center: center.geometry.coordinates, zoom: config.zoom }); // center map on start coordinates
...
function setMap(interval, speed, options) {
  ...
  if (step == null) {
    ...
    const easeToOptions = Object.assign({}, options.config.animationOptions, {
      center: step.coords,
    ...
    options.map.easeTo(easeToOptions);
